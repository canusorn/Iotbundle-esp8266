...
#define CONFIG_VERSION "0.1 beta"
...
IotWebConfParameterGroup login = IotWebConfParameterGroup("login", "ล็อกอิน(สมัครที่เว็บ iotkid.space ก่อนนะครับ)");
IotWebConfTextParameter emailParam = IotWebConfTextParameter("อีเมลล์", "emailParam", emailParamValue, STRING_LEN);
IotWebConfTextParameter passParam = IotWebConfTextParameter("รหัสผ่าน", "passParam", passParamValue, STRING_LEN);
...
void displayValue() {
  ...
  //display voltage
  oled.setCursor(3, 0);  oled.print(voltage, 1); oled.setCursor(42, 0); oled.println("V");

  //display current
  if (current < 10) oled.setCursor(9, 12);
  else oled.setCursor(3, 12);
  oled.print(current, 2); oled.setCursor(42, 12); oled.println("A");
  ...
  oled.print(power, 0); oled.setCursor(42, 24); oled.println("W");
  ...
  if (energy < 10) oled.print(energy, 3);
    ...
    oled.setCursor(8, 36);
    oled.print(energy, 0);
  } oled.setCursor(42, 36); oled.println("kWh");
  ...
  // display state
  if (iotWebConf.getState() == 1 || iotWebConf.getState() == 2) oled.drawIcon(55, 0, 9, 8, wifi_ap, sizeof(wifi_ap), true);
    ...
    if (iot.serverConnected) oled.drawIcon(56, 0, 8, 8, wifi_on, sizeof(wifi_on), true);
    else oled.drawIcon(56, 0, 8, 8, wifi_nointernet, sizeof(wifi_nointernet), true);
  }
  else if (iotWebConf.getState() == 5)  oled.drawIcon(56, 0, 8, 8, wifi_off, sizeof(wifi_off), true);
