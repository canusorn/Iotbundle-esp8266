...
static uint16_t NewshuntAddr1 = 0x0002;    // Discharge
static uint16_t NewshuntAddr2 = 0x0001;    // Charge
...
uint8_t displaytime, display_index;
String noti;
...
void display_update(float PZEMVoltage[2], float PZEMCurrent[2], float PZEMPower[2], float PZEMEnergy[2])
{
    ...
    oled.setCursor(0, 10);
    oled.print(PZEMVoltage[display_index]);
    oled.println(" V");
    oled.setCursor(0, 20);
    oled.print(PZEMCurrent[display_index]);
    oled.println(" A");
    oled.setCursor(0, 30);
    oled.print(PZEMPower[display_index]);
    oled.println(" W");
    oled.setCursor(0, 40);

    if (PZEMEnergy[display_index] < 9.999)
    {
        oled.print(PZEMEnergy[display_index], 3);
        oled.println(" kWh");
    }
    else if (PZEMEnergy[display_index] < 99.999)
    {
        oled.print(PZEMEnergy[display_index], 2);
        oled.println(" kWh");
    }
    else if (PZEMEnergy[display_index] < 999.999)
    {
        oled.print(PZEMEnergy[display_index], 1);
        oled.println(" kWh");
    }
    else if (PZEMEnergy[display_index] < 9999.999)
    {
        oled.print(PZEMEnergy[display_index], 0);
        oled.println(" kWh");
    }
    else
    {
    ...
    // on error
    if (isnan(PZEMVoltage[display_index]))
    {
        oled.clear(PAGE);
        oled.setCursor(0, 0);
        oled.printf("-Sensor-\n\nno sensor\ndetect!");
